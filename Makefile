...
alwaysdata: compile
	...
	$(COPY) output/ gbrunoro@ssh.alwaysdata.com:~/www/$(URL_PATH)

devious: compile
	@#ssh brunoro@devio.us "cd ~/public_html/; $(CREATE_PATH)"
	$(COPY) output/ brunoro@devio.us:~/public_html/$(URL_PATH)

dcc: compile
	@#ssh brunoro@login.dcc.ufmg.br "cd /www/users/grad/ccomp/09/brunoro/public_html/; $(CREATE_PATH)"
	$(COPY) output/ brunoro@login.dcc.ufmg.br:/www/users/grad/ccomp/09/brunoro/public_html/$(URL_PATH)
