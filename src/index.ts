...
const draw = () => {
    ...
    const drawReducer = (r: number, n: number, i: number): number => {
        ...
        poly.setAttribute('points', points.map(coords => coords.join(',')).join(' '));
...
const setAnimationRunning = (isRunning) => {
    animated = isRunning
...
const animateThreeSecs = () => {
    ...
    timeout = setTimeout(() => setAnimationRunning(false), 3000)
}

const toggleAnimation = (isRunning) => {
    if (timeout) {
        clearTimeout(timeout);
    }
